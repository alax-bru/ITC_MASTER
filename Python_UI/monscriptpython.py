# bootstrap_itc_v2.py - Version complète avec tous les onglets FEED
"""
Bootstrap ITC Manager V2 - Avec structure complète BASIC + FEED
Exécuter: python bootstrap_itc_v2.py
"""
...
# ===================== 2) schema_v2.json - Structure complète =====================
schema = {
    ...
    "tabs": {
        "basic": {
            ...
            "icon": "📊",
            "fields": [
                {"id": "decret", "label": "Décret", "type": "select", "options": ["Décret 92", "Décret 94"], 
                 "cell": "C2", "group": "Paramètres généraux"},
                {"id": "phase", "label": "Phase", "type": "select", "options": ["BASIC", "FEED", "REAL"], 
                 "cell": "C3", "group": "Paramètres généraux"},
                {"id": "mh_directes", "label": "Heures estimées directes Standard Construction", "type": "int", 
                 "cell": "J8", "ui_format": "thousands_MHrs", "placeholder": "10999999", "group": "Estimation des heures"},
                {"id": "facteur_prod", "label": "Facteur de productivité", "type": "float", 
        ...
        "feed_qty": {
            ...
            "fields": [
                ...
                {"id": "voirie_lourde", "label": "Surface Générale des Voirie Lourdes / General Area of heavy road", 
                 "type": "int", "cell": "J28", "ui_format": "thousands_m3", "group": "Surfaces et volumes"},
                {"id": "poids_equipment", "label": "Poids des Equipment à installer / Equipment Installation", 
                 "type": "int", "cell": "J29", "ui_format": "thousands_tons", "group": "Équipements"},
                ...
                {"id": "hvac", "label": "HVAC (Ml ou Tons)", 
                 "type": "int", "cell": "J31", "ui_format": "thousands_tons", "group": "Équipements"},
                {"id": "tuyauterie_pression", "label": "Tuyauterie sous pression U/G + tranchée", 
                 "type": "int", "cell": "J32", "ui_format": "thousands_tons", "group": "Tuyauterie"},
                {"id": "tuyauterie_gravit", "label": "Tuyauterie gravitationnelle U/G + tranchée", 
                ...
                {"id": "electricite", "label": "Électricité / Electricity", 
                 "type": "int", "cell": "J37", "ui_format": "thousands_km", "group": "Électricité"},
                {"id": "genie_civil", "label": "Travaux de Génie civil / Civil Works", 
                 "type": "int", "cell": "J38", "ui_format": "thousands_m3", "group": "Génie civil"},
                {"id": "pieux", "label": "Pieux / Piles", 
                 "type": "int", "cell": "J39", "ui_format": "thousands_ml", "group": "Génie civil"},
                {"id": "ignifugation", "label": "Ignifugation incendie / Fireproofing", 
                 "type": "int", "cell": "J40", "ui_format": "thousands_m2", "group": "Sécurité"},
                {"id": "structure_metal", "label": "Structure Métallique / Steel Structures", 
                 "type": "int", "cell": "J41", "ui_format": "thousands_tons", "group": "Structures"},
                {"id": "equipment_incendie", "label": "Equipment incendie / Fire Equipment", 
                 "type": "int", "cell": "J42", "ui_format": "thousands_tons", "group": "Sécurité"},
                {"id": "reservoir_tons", "label": "Réservoir / Tanks (Tons)", 
                 "type": "int", "cell": "J43", "ui_format": "thousands_tons", "group": "Réservoirs"},
                {"id": "reservoir_m3", "label": "Réservoir / Tanks (m³)", 
                 "type": "int", "cell": "J44", "ui_format": "thousands_m3", "group": "Réservoirs"},
                {"id": "plus_grand_reservoir", "label": "Le plus grand réservoir / Largest tank", 
                 "type": "int", "cell": "J45", "ui_format": "thousands_m3", "group": "Réservoirs"},
                {"id": "batiments_surface", "label": "Bâtiments / Buildings (surface à construire)", 
                 "type": "int", "cell": "J46", "ui_format": "thousands_m2", "group": "Bâtiments"},
                {"id": "batiments_empreinte", "label": "Bâtiments / Buildings (empreinte au sol)", 
                 "type": "int", "cell": "J47", "ui_format": "thousands_m2", "group": "Bâtiments"},
                ...
                {"id": "salles_blanches", "label": "Salles Blanches / White room", 
                 "type": "int", "cell": "J51", "ui_format": "thousands_m2", "group": "Spécialisé"}
            ]
        },
        "feed_surfaces": {
            "label": "FEED - Surfaces",
        ...
        "feed_planning": {
            ...
            "columns": [
                ...
                {"id": "p2", "label": "P2", "subtitle": "Gros Œuvre", "cell": "L15", "type": "date"},
                {"id": "p3", "label": "P3", "subtitle": "Second Œuvre ou Process", "cell": "L16", "type": "date"},
                {"id": "p4", "label": "P4", "subtitle": "PCC et Hand-Over", "cell": "L17", "type": "date"},
                {"id": "p5", "label": "P5", "subtitle": "À customiser si besoin", "cell": "L18", "type": "date"}
            ]
        }
    },
...
# ===================== 4) main.py (adapté pour nouvelle UI) =====================
main_code = r'''"""
...
# ===================== 5) ui/main_window_v2.py - Interface complète =====================
main_window_code = r'''"""
...
class BaseInputTab(QWidget):
    ...
    def create_widget(self, field: Dict[str, Any]) -> QWidget:
        """Créer le widget approprié selon le type"""
        t = field['type']
    ...
    def format_with_unit(self, widget: QLineEdit, unit: str):
        """Formater avec séparateurs de milliers et unité"""
        text = widget.text()
        # Enlever l'unité et les espaces
    ...
    def get_values(self) -> Dict[str, Any]:
        """Récupérer les valeurs pour écriture Excel"""
        vals = {}
...
class FeedSurfacesTab(QWidget):
    """Onglet spécial pour FEED - Surfaces avec tableau"""
    ...
    def format_surface(self, widget: QLineEdit):
        """Formater surface avec séparateurs"""
    ...
    def calculate_totals(self):
        ...
        # Mettre à jour les labels de récap
...
class FeedPlanningTab(QWidget):
    ...
    def update_readonly_dates(self, source_values: Dict[str, Any]):
        """Mettre à jour les dates en lecture seule depuis les sources"""
        for cell, info in self.widgets.items():
...
class InputsTabWidget(QTabWidget):
    ...
    def get_all_values(self) -> Dict[str, Any]:
        """Récupérer toutes les valeurs de tous les onglets"""
        all_values = {}
...
class DeliverablesTab(QWidget):
    ...
    def init_ui(self):
        ...
        desc = QLabel("Cliquez sur le bouton ci-dessous pour exécuter la macro de génération des livrables.\nCette action peut prendre plusieurs minutes.")
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        reports_layout.addWidget(QLabel("📊 Module Rapports - À venir"))
        self.main_tabs.addTab(reports, "📊 Rapports")
    ...
    def connect_excel(self):
        """Connexion Excel avec auto-détection"""
        try:
            ...
            if not excel_path or not p.exists():
                ...
                if not candidates:
                    QMessageBox.critical(self, "Erreur", "Aucun fichier .xlsm trouvé")
                    return
                p = candidates[0]
            ...
            # Mettre à jour le client dans les widgets
    ...
    def load_all_from_excel(self):
        ...
        try:
            # Récupérer toutes les cellules nécessaires
            all_cells = set()
    ...
    def save_all_to_excel(self):
        ...
        try:
            ...
            # Écrire en batch pour performance
    ...
    def run_macro(self):
        ...
        try:
            self.deliverables_tab.status_label.setText("⏳ Exécution de la macro en cours...")
            ...
            self.deliverables_tab.status_label.setText("✅ Macro exécutée avec succès!")
    ...
    def _toast_success(self):
        """Animation de succès"""
        toast = QLabel("🎉 Livrables générés avec succès!", self)
        toast.setStyleSheet("""
    ...
    def show_about(self):
        ...
        QMessageBox.about(self, "À propos", 
            ...
            "<p>Version complète avec modules FEED</p>"
            "<p>PySide6 + xlwings</p>")
...
print("✅ ui/main_window_v2.py (interface complète avec tous les onglets)")
...
print("\n💡 FONCTIONNALITÉS:")
print("  • Formats automatiques (m³, Tons, m², CV, Km, etc.)")
print("  • Calcul dynamique des totaux par WBS")
print("  • Dates readonly liées (L14 ← J14)")
print("  • Écriture batch pour performance")
