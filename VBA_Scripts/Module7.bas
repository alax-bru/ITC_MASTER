...
' ==========================================================================================================================================================
' ==========================================================================================================================================================
' Macros de navigation et de mise en forme de vues dans le fichier ITC MASTER
' Objectif : préparer des vues "métier" (WBS, PS, Annexes, Bilan...) pour l'utilisateur

Option Explicit

'------------------------------------------------------------------------------
' UnhideAllITCMASTER
' Remet la feuille WBS en vue complète (toutes lignes/colonnes visibles) + zoom standard.
' Utilisé comme "reset" après les vues filtrées / plein écran.
'------------------------------------------------------------------------------
Sub UnhideAllITCMASTER()

    ThisWorkbook.Sheets("01.3-ITC MASTER WBS").Activate

    ' Si un filtre est actif, le désactiver proprement
    On Error Resume Next
    ActiveSheet.ShowAllData
    On Error GoTo 0

    ' Afficher toutes les lignes/colonnes de la WBS
    Sheets("01.3-ITC MASTER WBS").Rows("1:5000").Hidden = False
    Sheets("01.3-ITC MASTER WBS").Columns("A:ZZ").Hidden = False

    ' Re-masquer les colonnes F:G si nécessaire (logic métier)
    Sheets("01.3-ITC MASTER WBS").Columns("F:G").Hidden = True

    ' Sortir du plein écran et recentrer la vue
...
'------------------------------------------------------------------------------
' UndoWBS
' Réinitialise la vue de la feuille "01.2-WBS & PIC" (toutes lignes/colonnes visibles).
'------------------------------------------------------------------------------
Sub UndoWBS()
...
'------------------------------------------------------------------------------
' UndoPSBOOS
' Réinitialise la vue de la feuille "2.7-PS ITC Global".
'------------------------------------------------------------------------------
Sub UndoPSBOOS()
...
'------------------------------------------------------------------------------
' PSBOOS
' Prépare une vue "boostée" / plein écran pour travailler sur la feuille PS ITC Global.
'------------------------------------------------------------------------------
Sub PSBOOS()
    ...
    ' Réinitialiser puis masquer les colonnes non pertinentes
...
'------------------------------------------------------------------------------
' WBSGraphic
' Prépare la vue graphique WBS & PIC (plein écran, zoom adapté).
'------------------------------------------------------------------------------
Sub WBSGraphic()
    ...
    ' Masque la ligne de titre / bandeau si souhaité
...
'------------------------------------------------------------------------------
' AnnexeA
' Affiche la vue d'entrée/suivi pour l'Annexe 1 (PP/SOW).
'------------------------------------------------------------------------------
Sub AnnexeA()
...
'------------------------------------------------------------------------------
' UndoAnnexeA
' Réinitialise la vue de l’Annexe 1 et sort du plein écran.
'------------------------------------------------------------------------------
Sub UndoAnnexeA()
...
'------------------------------------------------------------------------------
' AnnexeBInput
' Affiche la feuille d’INPUT pour l’Annexe 3 (Office Layout).
'------------------------------------------------------------------------------
Sub AnnexeBInput()
...
'------------------------------------------------------------------------------
' UndoAnnexeBInput
' Réinitialise la vue de la feuille d’INPUT Annexe 3 et sort du plein écran.
'------------------------------------------------------------------------------
...
'------------------------------------------------------------------------------
' AnnexeBOutput
' Affiche la feuille de sortie de l’Annexe 2 (texte structuré PP/SOW).
'------------------------------------------------------------------------------
Sub AnnexeBOutput()
...
'------------------------------------------------------------------------------
' UndoAnnexeBOutput
' Réinitialise la vue de l’Annexe 2 (sortie) et sort du plein écran.
'------------------------------------------------------------------------------
...
'------------------------------------------------------------------------------
' AnnexeC
' Affiche la feuille Annexe 3 (tableaux / scopes détaillés).
'------------------------------------------------------------------------------
Sub AnnexeC()
...
'------------------------------------------------------------------------------
' UndoAnnexeC
' Réinitialise la vue de l’Annexe 3 et sort du plein écran.
'------------------------------------------------------------------------------
Sub UndoAnnexeC()
...
'------------------------------------------------------------------------------
' AnnexeD
' Affiche la feuille dédiée à l’Annexe 4 (layout TSF / plan).
'------------------------------------------------------------------------------
Sub AnnexeD()
...
'------------------------------------------------------------------------------
' UndoAnnexeD
' Réinitialise la vue de l’Annexe 4 et sort du plein écran.
'------------------------------------------------------------------------------
Sub UndoAnnexeD()
...
'------------------------------------------------------------------------------
' SiteVisitDoc
' Affiche la vue dédiée au "Site Visit Doc" (questionnaire de visite de site).
'------------------------------------------------------------------------------
Sub SiteVisitDoc()
...
'------------------------------------------------------------------------------
' UndoSiteVisitDoc
' Réinitialise la vue du Site Visit Doc et sort du plein écran.
'------------------------------------------------------------------------------
...
'------------------------------------------------------------------------------
' QTYDurMhrs
' Crée une vue WBS centrée sur quantités/durée/man-hours (inputs principaux).
'------------------------------------------------------------------------------
Sub QTYDurMhrs()
    ...
    With ThisWorkbook.Sheets("01.3-ITC MASTER WBS")
        ...
        ' Masquer tout puis réafficher uniquement la zone pertinente
        ...
        ' A:H -> A:J après insertion de 2 colonnes
...
'------------------------------------------------------------------------------
' BilanInput
' Affiche la feuille de Bilan ITC MASTER par famille (vue complète).
'------------------------------------------------------------------------------
Sub BilanInput()
...
'------------------------------------------------------------------------------
' InputClient
' Configure la vue WBS pour revue des inputs client (zones colonnes B:Q & W:AB).
'------------------------------------------------------------------------------
Sub InputClient()

    StatusBar = True              ' Mécanisme existant conservé
    Application.ScreenUpdating = False
...
'------------------------------------------------------------------------------
' PriceEstimation
' Vue dédiée à l’estimation de prix (colonnes de coûts/coeffs visibles).
'------------------------------------------------------------------------------
Sub PriceEstimation()
    ...
    With ThisWorkbook.Sheets("01.3-ITC MASTER WBS")
        ...
        ' Colonnes coûts/prix/coeff visibles
        ...
        ' Masques fins sur certaines lignes (non utilisées)
...
'------------------------------------------------------------------------------
' ReportingGraphique
' Vue optimisée pour le reporting graphique (1.4-Bilan Graphique).
'------------------------------------------------------------------------------
...
'------------------------------------------------------------------------------
' UnhideAllGraphique
' Réinitialise la feuille "1.4-Bilan Graphique" (toutes lignes/colonnes visibles).
'------------------------------------------------------------------------------
...
'------------------------------------------------------------------------------
' MacroARenommer
' Vue WBS filtrée sur quelques colonnes pour une analyse spécifique (nom à ajuster).
'------------------------------------------------------------------------------
Sub MacroARenommer()
    ...
    With ThisWorkbook.Sheets("01.3-ITC MASTER WBS")
        ...
        ' Masquer tout sauf B,C,D,I,J,L:R,AD:AF (après décalages)
        ...
        ' Masquer les lignes hors zone d'intérêt
        .Rows("1:165").Hidden = True
    End With

    Range("A1").Select
    Application.GoTo Reference:=Range("A1"), Scroll:=True
    Range("I9").Select   ' I9 après décalage colonnes
    ActiveWindow.Zoom = 50
...
'------------------------------------------------------------------------------
' BilanNumerique
' Vue WBS dédiée au bilan numérique (quantités globales par famille).
'------------------------------------------------------------------------------
Sub BilanNumerique()
    ...
    ' Colonnes clés visibles (après décalage indices)
    ...
    Sheets("01.3-ITC MASTER WBS").Columns("H").Hidden = False   ' F -> H
    Sheets("01.3-ITC MASTER WBS").Columns("J").Hidden = False   ' H -> J
    Sheets("01.3-ITC MASTER WBS").Columns("K").Hidden = False   ' I -> K
    Sheets("01.3-ITC MASTER WBS").Columns("BW:CA").Hidden = False   ' BU:BY -> BW:CA
    Sheets("01.3-ITC MASTER WBS").Columns("CC:CK").Hidden = False   ' CA:CI -> CC:CK

    ' Lignes d'agrégats visibles
    ...
    ' Ajuste la hauteur des lignes-clés
...
'------------------------------------------------------------------------------
' BilanManpower
' Vue WBS centrée sur le bilan de main-d’œuvre (manpower).
'------------------------------------------------------------------------------
Sub BilanManpower()
...
'------------------------------------------------------------------------------
' SurfaceDispo
' Vue WBS dédiée aux surfaces disponibles (surface par zone).
'------------------------------------------------------------------------------
Sub SurfaceDispo()
...
'------------------------------------------------------------------------------
' SurfaceEstimee
' Vue WBS dédiée aux surfaces estimées (outputs de calcul).
'------------------------------------------------------------------------------
Sub SurfaceEstimee()
...
'------------------------------------------------------------------------------
' BilanSurfaces
' Vue WBS synthétique sur le bilan des surfaces (ligne d’agrégat dédiée).
'------------------------------------------------------------------------------
Sub BilanSurfaces()
...
'------------------------------------------------------------------------------
' BilanEnergies
' Vue WBS de synthèse sur les bilans énergétiques (demandes par zone).
'------------------------------------------------------------------------------
Sub BilanEnergies()
...
'------------------------------------------------------------------------------
' BilanEnergieEncore
' Crée une feuille temporaire "VueTemporaire" avec un ordre de lignes priorisé
' pour relecture rapide des bilans énergétiques.
'------------------------------------------------------------------------------
Sub BilanEnergieEncore()
    ...
    ' Nettoyer les anciennes vues temporaires si elles existent
    On Error Resume Next
    ...
    ' Ordre de lignes à mettre en priorité
    ...
    ' 1) Copier d'abord les lignes prioritaires dans l'ordre voulu
    ...
    ' 2) Puis compléter avec les autres lignes visibles
    ...
    ' Masquage de certaines colonnes techniques pour lecture plus claire
...
'------------------------------------------------------------------------------
' SelectionMarchesTravaux
' Vue WBS centrée sur la sélection des marchés de travaux (Partie B).
'------------------------------------------------------------------------------
...
'------------------------------------------------------------------------------
' ServiesSpecifiquesPar
' Vue WBS pour les services spécifiques par marché (filtres colonnes K, P:R).
'------------------------------------------------------------------------------
...
'------------------------------------------------------------------------------
' PhasagePartieB
' Vue WBS dédiée au phasage Partie B (colonnes V:AF visibles).
'------------------------------------------------------------------------------
Sub PhasagePartieB()
...
'------------------------------------------------------------------------------
' PhasagePartieA
' Vue WBS dédiée au phasage Partie A (lignes 10:17, colonnes L:R).
'------------------------------------------------------------------------------
Sub PhasagePartieA()
...
'------------------------------------------------------------------------------
' PhasagePartA
' Variante de la vue Phasage Partie A (même zone, sans repositionnement final).
'------------------------------------------------------------------------------
Sub PhasagePartA()
...
'------------------------------------------------------------------------------
' PPBOOS
' Vue "boostée" sur la Partie PP (préparation contrats / marchés) dans la WBS.
'------------------------------------------------------------------------------
Sub PPBOOS()
